...
router.get("/:Id", function (req, res) {
    ...
    db_1.pool.query("select m.id,m.firstname, m.middlename, m.lastname,m.email,m.phoneno,m.address,c.customer_name, r.name AS ROLE,m.createdon,m.modifiedon from mydata m JOIN customer c ON m.id=c.customer_id JOIN role r ON r.role_key=m.role_key WHERE id=$1;", [id]).then(function (userData) {
        res.status(200).send(userData.rows);
    });
});
